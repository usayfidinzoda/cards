...
```
{
    "external_ref": "130444",
    "code": 997,
    "message": "Номер телефона, привязанный к карте, отличается от номера кошелька. Повторите попытку через кошелёк, зарегистрированный на привязанный к карте номер",
    "status": "failed"
}
```
...
Если отправите поле телефона пустым, то будет отправлено сообщение с OTP на номер к привязанной карте в банке. 
